...
def visualize_actions(action_heatmap):
    ...
    for row in range(tu.GRID_ROWS):
        for col in range(tu.GRID_COLS):
            ...
            direction = tu.ACTION_TO_DIR.get(action_heatmap[row][col], (0, 0))
            if direction == (0, 0):
                continue
...
def _visualize():
    """Template for using the visualization functions.
    """
    ...
    visualize_adjacency()
    visualize_locations()
    visualize_taxi_passenger_destination(344)
    # visualize_actions()
    # visualize_options()
    visualize_options_bubble_plot(option_values=None, option_labels=None)
